...
const seedListings = () => {
  for (let i = 1; i <= 100; i += 1) {
    pool.query('INSERT INTO listings (listings) VALUE (?)', [`L${i}`], (error) => {
...
const seedImages = () => {
  for (let i = 1; i <= 100; i += 1) {
    ...
    pool.query('INSERT INTO images (url, description, user_submit, date, unrelated_flag, inappropriate_flag, dislike_flag) VALUE (?, ?, ?, ?, ?, ?, ?)', [imageUrl, words, faker.random.boolean(), faker.date.past(), 0, 0, 0], (error) => {
      if (error) { console.log(error); }
    });
  }
};
...
const seedListingsImages = () => {
  ...
  while (amountOfImages < 1000) {
    ...
    pool.query('INSERT INTO listings_images (listing_id, image_id) VALUE (?, ?)', [randListingId, randImageId], (error) => {
      if (error) { console.log(error); }
    });
